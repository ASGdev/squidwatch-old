...
class PuppeteerCrawler extends EventEmitter {
  ...
  async navigate (url) {
    ...
    try {
      ...
      logger.logError("Crawler encountered a navigation error (" + e + ") for URL " + this._currentUrl)
      return false
    }
    this._pages.push(this._currentUrl)
    return true
  ...
  async runUserScript () {
	...
	logger.logError("Error while running the user script (" + e + ") for URL " + this._currentUrl)
      }
    } else {
      try {
	...
	logger.logError("Error while running the default scroll script (" + e + ") for URL " + this._currentUrl)
      }
    }
   ...
   */
  _onWARCGenError (err) {
    logger.logError("Error generating WARC file for URL " + this._currentUrl)
   ...
   */
  _onWARCGenFinished () {
    logger.logInfo("Warc generation finished for URL " + this._currentUrl)
