...
class Frontier {
  ...
  constructor () {
    ...
    // for logging - processed items count
    this.processed = 0
  }
   ...
   */
  exhausted () {
    logger.logInfo("Queue is empty now");
  ...
  next () {
    ...
    if (this.current) {
      logger.logInfo("Selected URL " + this.current.url)
  ...
  _processSiteMode (links) {
    ...
    logger.logInfo("Adding " + tmp_logger_value + " URL to queue")
  }
